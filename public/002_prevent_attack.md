---
title: ペンテストに備えるIoTデバイスの守備固め（侵入対策・DoS対策）
tags:
  - Security
  - devops
  - yocto
  ...
  - 組み込みLinux
private: true
updated_at: '2026-02-24T13:40:39+09:00'
...
私はShizen Connectで、エネルギーマネジメントシステムを支えるエッジ端末「Shizen Box」を開発しています。Shizen Boxは、太陽光発電や蓄電池などの複数のプロトコル機器との接続・連系制御を担うデバイスです。
...
Shizen Boxは、一般社団法人環境共創イニシアチブが定める[JC-STAR（Japan Cybersecurity Standards, Certification Scheme for Advanced & Trusted IoT）](https://sii.or.jp/jc-star/)の★1認証を取得しています。
本記事で紹介する対策は、ペンテスト対応だけでなく、JC-STARやERABを見据えたセキュリティ要件を出来る限りカバーすることを目指して事前対策を実施しました。
...
通常の組み込み Linux 開発では、ベンダー提供の BSP（Board Support Package：ボードサポートパッケージ）をベースに自社用の rootfs を構築するケースが多いです。この BSP には、製品用途では不要なパッケージやサービスが初期状態で含まれていることが少なくありません。
...
ファイアウォール設定が正しく機能しているかを確認するため、設定前後で Nmap によるスキャンを実施し意図せず開いているポートが無いかを確認します。
...
理想的には SYN スキャンに対しても完全に不可視としたかったのですが、
コストと実現性を考慮し、本構成では
...
この設定により、シリアルポートからの対話的ログインができなくなります。
...
キッティング後の再検査が必要なケースに備え、製造拠点でのみ使用可能な復帰手順を用意しています。これにより、フィールドでの物理アクセスによる不正ログインリスクを低減しつつ、メンテナンス性を維持する設計としました。
...
第一回で紹介したフルスタック開発環境を活用し、カーネルソースの `.config` または `defconfig` を編集してビルドしています。
...
意図的に破損させたパケットや異常なフラグ構成による負荷を軽減するため、
