...
昨年、Shizen Boxは第三者ペネトレーションテスト（以下、ペンテスト）を実施しました。本記事では、私がペンテスト対応した実務経験をもとに、Shizen Box におけるフルスタック開発環境の設計と運用について説明します。
...
- ペンテスト対策や指摘に対する修正をフルスタックで迅速に実施する必要があった
...
Gitの内部の仕組み（Treeオブジェクト）に関するドキュメントにおいて、ファイル（blob）に対して保持されるモードが厳格に限定されていることが説明されています。
...
運用重視の開発環境構築に取り組んでいる方の参考になれば幸いです。
