...
ある大規模蓄電所案件において、eMMCが半年程度で急速に劣化するという問題が発生しました。本記事では、原因の特定から対策の実測評価、Linuxカーネルパラメータによる追加最適化まで、実際の計測データをもとに解説します。

## 問題の背景

Shizen Boxが採用するeMMC（4GB pSLC、書き込み耐久30,000サイクル）の理論寿命は約120TBです。通常の使用であれば数年は持つはずでした。

ところが大規模案件（約5,000 IO接続）では、約6ヶ月で劣化兆候が現れるという問題が起きました。
...
Shizen Boxのアプリケーションは、接続されたIOごとに2種類のファイルを管理しています。

| ファイル | 役割 | 実際のサイズ |
|---------|------|----------|
| dat ファイル（IO毎に1個） | 各IOの最新計測値・状態を保持 | 72 〜 196,652 バイト（IOの種類による） |
| `mringf_info`（1個） | 全IO分の管理情報をまとめたリングバッファ | 約440KB（5,000 IO × 88バイト固定） |

:::note
評価ツール `emmc_test` では計測しやすくするためファイルサイズを固定値（dat: 64バイト、info: 440KB）で仮実装しています。dat ファイルの実サイズは IO の種類によって幅がありますが、問題の主因は `mringf_info` の書き込み頻度であり、dat ファイルのサイズは評価の結論に影響しません。
:::

### 原因の特定

調査の結果、アプリケーション内で管理情報ファイル（`mringf_info`、約440KB）を、接続されている全IOのデータ更新のたびに毎回書き込んでいたことが原因でした。

5,000 IOが頻繁に更新される環境では、
...
`mringf_info` は `fsync()` を呼ぶため、コアレッシングが一切起きずに毎回 eMMC への物理書き込みが発生します。一方、dat ファイルは `fclose()` のみのためページキャッシュに滞留し、カーネルパラメータの影響を受けます。
...
|---------|-----------|------------|---------------------|
| mringf_info（440KB） | fflush → fsync → 即時ライトバック | **なし** | **なし** |
...
| 対策 | 内容 |
|-----|------|
| コード最適化 | `mringf_info` の書き込み頻度を「毎IO」から「30秒ごと1回」に変更 |
| カーネルパラメータ最適化 | dirty page の書き込みタイミングを調整 |

### コード最適化

**修正前（mode 0）: 書き込みスレッドが dat ファイル更新のたびに毎回 `mringf_info` を fsync**
...
void *write_thread(void *arg) {
    ...
    while (g_running) {
        for (int id = targ->start_id; id <= targ->end_id && g_running; id++) {
            ...
            // Mode 0: dat ファイル更新のたびに毎回 mringf_info を書き込む（fsync あり）
            if (targ->mode == 0) {
...
**修正後（mode 1）: モニタースレッドが 30 秒に 1 回だけ `mringf_info` を fsync**

```c
// モニタースレッド: 30秒ごとに1回だけ mringf_info を書き込む（mode 1 のみ）
...
- **モード0（修正前）**: 5,000ファイルの更新のたびに毎回 `mringf_info` を書き込む（fsync）
- **モード1（修正後）**: 30秒ごとに1回だけ `mringf_info` を書き込む（fsync）
...
sectors_start=$(awk -v dev="mmcblk2" '$3==dev {print $10}' /proc/diskstats)
# ... テスト実行 ...
sectors_end=$(awk -v dev="mmcblk2" '$3==dev {print $10}' /proc/diskstats)
...
`mringf_info` ファイルへの書き込みには `fsync()` を使っているため、ページキャッシュを完全にバイパスします。一方、dat ファイル（64バイト）は `fclose()` のみでページキャッシュ経由です。

| ファイル | 書き込み方法 | ページキャッシュ経由 | カーネルパラメータの影響 |
|---------|-----------|-----------------|---------------------|
| mringf_info（440KB） | `fsync()` | しない | **なし** |
| dat ファイル（64バイト） | `fclose()` のみ | する | **あり** |

mode 0/1 の差（96%削減）は `mringf_info` の `fsync()` 回数が支配的なため、カーネルパラメータを変えても「Total write amount」の数値はほとんど変わりません。
...
17.50 MB の内訳は主に `mringf_info` の fsync（2回 × 440KB ≈ 0.86 MB）とファイルシステムのメタデータ・ジャーナルです。
...
- 特定の製品仕様、社内限定情報、実環境を特定可能な情報は含んでいません。
